...
#else
//success = ARM_MakeExecutable(region.begin(), region.size());
...
bool WasmCodeManager::ARM_SetDomainAndDACR(Address address, size_t size, int domain_val) {
    int fd;
    ...
    struct wasdom_user_input_dacr input_dacr; 
    input_dacr.domain_val = domain_val;
    input_dacr.dacr_val = dacr_val;

    // DACR 설정
    temp = ioctl(fd, WASDOM_CMD_WR_DACR, &input_dacr); 
    if (temp < 0) {
...
/*
int WasmCodeManager::calculate_index(size_t size) {
    int shift_size = 0;
    while (size > 1) {
        shift_size++;
        size >>= 1; 
...
*/
int WasmCodeManager::domain_bits_get(Address address) {
    int fd, temp;
    int domain_val;
    ...
    if (temp < 0) {
        printf("Error reading domain in wasdom_driver\n");
        close(fd);
        return -1;
    }
...
void WasmCodeManager::Record_Table(Address address, size_t size, size_t hint, int domain_bits) {
    int fd;
    int32_t temp;
...
//Check_table은 code_vault_index를 인자로 받아가고, code_vault라는 구조체를 반환함. 구조체에는 address, end, hint, code_vault_index가 있음.
// 메모리 정보가 있다면 해당 정보(code_cault.hint)를 이용하여 hint를 설정
...
std::shared_ptr<NativeModule> WasmCodeManager::NewNativeModule(
  ...
  for (int retries = 0;; ++retries) {
    code_space = TryAllocate(code_vmem_size, code_vault_index);
...
int WasmCodeManager::get_from_domain_table(Address address) {
...
std::vector<std::unique_ptr<WasmCode>> NativeModule::AddCompiledCode(
  ...
  //JIT_Writable writable("JIT_Writable", domain_val);
  //nesting_level과 같은 MPK에서 사용하는 변수들을 사용하여 write scope를 설정
