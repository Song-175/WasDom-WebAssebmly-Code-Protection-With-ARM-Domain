...
*/
 ...
 private:
  ...
  V8_WARN_UNUSED_RESULT VirtualMemory TryAllocate(size_t size, int code_vault_index,
                                                  void* hint = nullptr);                                      
  ...
  // 가정: code_vaults_와 같은 인덱스를 사용하는 별도의 배열
  std::array<size_t, 16> code_vault_used_space_;

  // 코드 공간의 할당을 추적하는 함수
  void RegisterCodeSpace(int code_vault_index, const VirtualMemory& mem) {
      if (code_vault_index >= 0 && code_vault_index < static_cast<int>(code_vault_used_space_.size())) {
          code_vault_used_space_[code_vault_index] += mem.size();
      }
  }
   // 특정 code vault 내에 할당된 모든 code spaces의 총합 크기를 반환하는 함수
  size_t CalculateUsedSpaceInCodeVault(int code_vault_index) const {
      if (code_vault_index >= 0 && code_vault_index < static_cast<int>(code_vault_used_space_.size())) {
          return code_vault_used_space_[code_vault_index];
      }
      return 0; // 잘못된 인덱스인 경우 0 반환
  }
...
public:
    ...
    ~JIT_Writable() {
        if (!v8::internal::v8_flags.jitless) {
            Set_DACR_Executable(domain_val);
        }
    }

private: 
    int32_t domain_val; // 생성자에서 받은 domain_val을 저장하기 위한 멤버 변수
    int code_space_write_nesting_level_ = 0;
    
    void Set_DACR_Writable(int domain) {
        int fd;
        //int32_t dacr_val; // 로컬 변수로 선언
        ...
        struct wasdom_user_input input_dacr;
          input_dacr.domain_val = domain;
          input_dacr.dacr_val = WASDOM_DACR_MANAGER;

        // 현재 DACR 값을 가져옴 
        //if (ioctl(fd, WASDOM_CMD_RD_DACR, &input_dacr) < 0) {
        //     perror("Error reading DACR in wasdom_driver");
        //    close(fd);
        //    return;
        //}
        //dacr_val = input_dacr.dacr_val;
        ...
        if (code_space_write_nesting_level_ == 0) {
            ...
            // DACR 값을 적용합니다.
            if (ioctl(fd, WASDOM_CMD_WR_DACR, &input_dacr) < 0) {
                printf("Error setting DACR in wasdom_driver");
                close(fd);
                return;
            }
    ...
    void Set_DACR_Executable(int domain) {
        int fd;
        //int32_t dacr_val; // 로컬 변수로 선언
        code_space_write_nesting_level_--;
        fd = open("/dev/wasdom_device", O_RDWR);
        ...
        struct wasdom_user_input input_dacr;
          input_dacr.domain_val = domain;
          input_dacr.dacr_val = WASDOM_DACR_CLIENT;
